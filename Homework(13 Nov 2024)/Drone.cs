...
public class Drone : MonoBehaviour
{
    ...
    private void Update()
    {
        Coolness = UnityEngine.Random.Range(0, 10000); // Assign random Coolness
        UpdateVisualColor(); // Update color based on the new Coolness value
    }

    public void Initialize(Flock flock, int index)
    {
        agentFlock = flock;
        ID = index; // Assign the ID based on the index of the drone in the agents array
